...
export default function RoomSystem(socket: Socket)
{
    ...
    socket.on("req-entry", (targetRoom: string, targetUser: UserProps) =>
    {
        RoomList.forEach(room =>
        {
            ...
            {
                if (!room.participants.some(participant => participant.name.toUpperCase() === targetUser.name.toUpperCase()) && !room.pending.includes(targetUser))
                {//? Make sure that the username is not used on the room or pending list
                    room.pending.push(targetUser) //? Add the user to the pending list
                } else
                { //? This function will trigger when username is already existed in the room or pending list
    ...
    {
        RoomList.forEach(room =>
        { //? VALIDATION (Make sure the username is not existed in the room)
            if (room.id === roomID)
            { //? Find the specific room
                if (room.participants.some(participant => participant.name.toUpperCase() === userInfo.name.toUpperCase()))
                { //? If username existed
                    io.to(socket.id).emit("user-existed")
                } else
                { //? Join room if username is not existed
                    if (((userInfo.IPv4 === room.host.IPv4) && (userInfo.name.toUpperCase() === room.host.name.toUpperCase())))
                    { //? Incase host got reconnected (New ID)
                ...
                // TODO : Fix this its too fast for the client to catch
                // if (room.exam.active)
                //     setTimeout(() => io.to(socket.id).emit("ExamAvailable", roomID), 1000);
                // if (room.stream.presenting)
                // { //? If a late comer join the meeting while someone is presenting
                //     setTimeout(() =>
                //     {
    ...
    socket.on("session-loaded", (targetRoom: string) =>
    {
        const room = RoomList.find(room => room.id === targetRoom)!
        if (room.exam.active)
            io.to(socket.id).emit("ExamAvailable", targetRoom)
        if (room.stream.presenting)
        {
            io.to(room.stream.hostID ?? room.stream.streamer?.id).emit("get-stream", socket.id)
            io.to(socket.id).emit("streaming")
    ...
    socket.on("leave-room", (targetRoom: string) =>
    {
        RoomList.forEach(room =>
        {
            ...
            { //? Find the target room
                ...
                { //? If a participant leave the meeting
